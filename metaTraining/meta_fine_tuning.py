...
N_WAY = 5
K_SHOT = 1
IS_PERSONAL = True  # Toggle: True for 85% target, False for 68% target
...
def test():
    ...
    for current_seed in range(10):
        ...
        base_model.load_state_dict(torch.load(f"meta_weights/meta_model_seed_{current_seed}.pth"))
        base_model.eval() 
        ...
        for episode, batch in enumerate(loader):
            ...
            for _ in range(FINE_TUNE_STEPS):
                ...
                # Use torch.no_grad() for the backbone to save memory/speed
                with torch.no_grad():
                    emb = model(support_x)
